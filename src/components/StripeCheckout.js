...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async() => {
   try {
     ...
     return {
       statusCode: 500,
       body : JSON.stringify({msg: error.message, success: false})
     }
   }
  }
  ...
  const handleSubmit = async(ev) => {
    ...
    if(payload.error) {
      setError(`Payment is fail ${payload.error.message}`);
      setProcessing(false)
    } else {
      setError(false)
      setProcessing(false)
      ...
      // redirecting to checkout page after the payment succesfull
      setTimeout(() => {
        clearCart()
  ...
  return (
    <div>
      ...
      <form id='payment-form' onSubmit={handleSubmit}>
        ...
        {/* show error if there is any when processing payment */}
        {error && <div className='card-error' role='alert'>{error}</div>}

        {/* show succeded message when payment is succesfull */}
